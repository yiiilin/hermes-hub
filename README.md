...
```bash
HERMES_HUB_BACKEND_IMAGE=ghcr.io/yiiilin/hermes-hub:0.0.3
...
```bash
docker pull ghcr.io/yiiilin/hermes-hub:0.0.3
```
...
Releases are tag-driven. Pushing a tag such as `v0.0.3` triggers the release workflow, builds the Docker image, pushes GHCR tags, and creates a GitHub Release with the commit list.
